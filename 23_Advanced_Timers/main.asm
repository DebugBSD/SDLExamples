...
main proc 
	...
	.while quit!=1
		...
		.while rax!=0
			...
			.elseif eventHandler.type_ == SDL_KEYDOWN
				.if eventHandler.key.keysym.sym == SDLK_s
					...
					invoke LTimer_IsStarted, addr timer
					.if rax==1
						invoke LTimer_Stop, addr timer
					.else
						invoke LTimer_Start, addr timer
					.endif
					
				.elseif eventHandler.key.keysym.sym == SDLK_p
					invoke LTimer_IsPaused, addr timer
					.if rax == 1
						invoke LTimer_Unpause, addr timer	
					.else
						invoke LTimer_Pause, addr timer
					.endif
				.endif
			.endif
		...
		; Get current time
		invoke LTimer_GetTicks, addr timer
		mov rdx, 0
		mov rbx, 1000
		idiv rbx
