...
; Constructor
LTexture_ctor proc uses rdi, pTexture:QWORD
...
LTexture_dtor proc uses rdi, pTexture:QWORD
	mov rdi, pTexture
...
;	- N/A
;
renderTexture proc uses rsi rax rbx rcx rdx, gRenderer:QWORD, pTexture:QWORD, x:DWORD, y:DWORD, clip:PTR SDL_Rect,angle:REAL8,center:QWORD,flip:SDL_RendererFlip
...
LTexture_setAlpha proc uses rsi rbx, pTexture:QWORD, alpha:BYTE
	
	mov rsi, pTexture
	mov rbx, (LTexture PTR [rsi]).m_Texture
	
	invoke SDL_SetTextureAlphaMod, rbx, alpha
	ret
LTexture_setAlpha endp

loadTextureFromRenderedText proc uses rbx rsi rdi, pRenderer:ptr, pTexture:ptr LTexture, pFont:ptr, pTextureText:ptr, textColor:SDL_Color
	LOCAL textSurface:QWORD
	
	invoke LTexture_dtor, pTexture
	
	; Render text surface
	invoke TTF_RenderText_Solid, pFont,  pTextureText, textColor
	.if rax==0
		jmp EXIT	
	.endif
	...
	; Create texture from surface pixels
	invoke SDL_CreateTextureFromSurface, pRenderer, rax
	.if rax==0
		jmp EXIT
	.endif
	
	; Copy data to our variables.
	mov rdi, pTexture
	mov (LTexture PTR[rdi]).m_Texture, rax
	mov rsi, textSurface
	mov eax, (SDL_Surface PTR [rsi]).w
	mov ebx, (SDL_Surface PTR [rsi]).h
	mov (LTexture PTR[rdi]).m_Width, eax
	mov (LTexture PTR[rdi]).m_Height, ebx
	
	; Get rid of old surface
	invoke SDL_FreeSurface, textSurface
	
	; Everything OK
	mov rax, 1
EXIT:
	ret
loadTextureFromRenderedText endp